...
- 为每篇作品生成可直接打开的知乎标题检索入口；只有实际发现并验证的直链才写入 `work_url`，不把公众号文章链接冒充成知乎原文；
...
- 从已有归档生成 `rank_history` 和 `rank_changes`，识别连续上榜、掉榜、异军突起和排名变化；
...
这个公众号的榜单正文目前只给出作品标题和简介，没有给每篇作品嵌入知乎直链。因此输出会分成两种：

- `work_url`：只有抓到并验证过的知乎原文直链才填写；
- `work_search_url`：没有直链时，生成知乎站内标题检索入口，供网页版 GPT 继续定位同名原文。

这样不会把公众号文章地址错误地当作作品原文，也不会凭标题猜一个可能错误的知乎页面。
